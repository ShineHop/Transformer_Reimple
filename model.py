...
class PositionalEncoding(nn.Module):
    def __init__(self, dim_model, max_len, device):
        ...
        self.token_embedding = nn.Embedding(num_embeddings=52000, embedding_dim=dim_model, padding_idx=1, device=device)   # num_embeddings : vocab size

        self.pos_encoding = torch.zeros(max_len, dim_model, device=device)

        pos = torch.arange(0, max_len, dtype=torch.float, device=device).view(-1, 1)  # pos = tensor([0, 1, 2, 3, 4, 5]) -> (max_len, 1)의 2차원 tensor : 2차원이면서 두번째 차원은 1인 tensor 
    ...
    def forward(self) -> torch.tensor:    # token_embedding을 torch.tensor로 받고, output 또한 torch.tensor로 내보냄
        seq_len = self.token_embedding.size()

        # token_embedding + pos encoding
        return self.token_embedding + self.pos_encoding[:seq_len, :]


class MultiHeadAttention(nn.Module):    
    def __init__(self, dim_model, num_heads, device):
...
class FeedForward(nn.Module):       
    def __init__(self, dim_model, hidden, device):
...
class EncoderLayer(nn.Module):      # 6 layers
    def __init__(self, dim_model, ffn_hidden, num_heads, device):
        super(EncoderLayer, self).__init__()
        self.dim_model = dim_model
        self.ffn_hidden = ffn_hidden
        self.num_heads = num_heads

        self.multihead_attention = MultiHeadAttention(self.dim_model, self.num_heads, device=device)
        self.normalization1 = nn.LayerNorm(dim_model)

        self.feedforward = FeedForward(self.dim_model, self.ffn_hidden, device=device)
...
class Encoder(nn.Module):   # num_layers = 6
    def __init__(self, enc_voca_size, max_len, dim_model, ffn_hidden, num_heads, num_layers, device):
        super(Encoder, self).__init__()

        # self.embedding = nn.Embedding()
        self.pos_encoding = PositionalEncoding(dim_model=dim_model, max_len=max_len, device=device)
        # multi layers
...
class DecoderLayer(nn.Module):      # 6 layers
    def __init__(self, dim_model, ffn_hidden, num_heads, device):
        ...
        self.masked_self_attention = MultiHeadAttention(self.dim_model, self.num_heads, device=device)   # masked 필요
        self.normalization1 = nn.LayerNorm(dim_model)

        self.enc_dec_attention = MultiHeadAttention(self.dim_model, self.num_heads, device=device)
        self.normalization2 = nn.LayerNorm(dim_model)

        self.feedforward = FeedForward(self.dim_model, ffn_hidden, device=device)
...
class Decoder(nn.Module):   # num_layers = 6
    def __init__(self, dec_voca_size, max_len, dim_model, ffn_hidden, num_heads, num_layers, device):
...
class Transformer(nn.Module):
    def __init__(self, dim_model, num_heads, max_len, ffn_hidden, num_layers, enc_voca_size, dec_voca_size, device):
